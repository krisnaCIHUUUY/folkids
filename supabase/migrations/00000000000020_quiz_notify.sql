-- Notifikasi 'kuis_dinilai': beri tahu pembuat kuis saat siswa menyelesaikannya.
-- Mengganti submit_quiz_attempt (00015) dengan fungsi identik + satu INSERT
-- notifikasi di akhir. Fungsi sudah SECURITY DEFINER sehingga boleh menulis ke
-- notifications (yang tak punya policy INSERT).
...
begin
  ...
  -- Notifikasi ke pembuat kuis bahwa siswa telah menyelesaikan kuis.
  insert into public.notifications (user_id, type, title, body, link)
  select q.created_by, 'kuis_dinilai',
         u.name || ' menyelesaikan ' || q.title,
         'Nilai ' || v_total || '/' || v_max, '/asesmen'
